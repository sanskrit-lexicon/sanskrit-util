...
The "Re-check recipe" column forward-references RECIPES §s that Wave 2 fills; until then every
row emits `RECIPES §TBD`.
"""
import argparse
import re
import sys
...
def main():
    ...
    out.append("")
    out.append(f"_Created: {iso_to_dmy(today)} · Last updated: {iso_to_dmy(today)}_")
    out.append("")
    ...
    out.append("> **Discovered** = earliest date in the finding's block · **Last re-verified** = "
               ...
               "**Re-check recipe** = the RECIPES row that reproduces this finding (Wave 2 fills "
               "the numbers; `§TBD` until then).")
    out.append("")
    ...
    for num, title, discovered, last, age, anchor in rows:
        ...
        out.append(f"| [§{num}]({link}) | {d_str} | {l_str} | {a_str} | {fl} | RECIPES §TBD |")
    out.append("")
